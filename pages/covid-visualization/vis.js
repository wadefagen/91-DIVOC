...
var prep_data = function(chart) {
  ...
  switch (chart.show) {
    ...
    case "highlight-only":
      caseData = _.filter(caseData, function(d) {
        return d.country == chart.highlight;
      });
      break;
  }
  ...
  if ($highlight.html().length < 100) { 
    ...
    for (var country of allCountries) {
      var el = $("<option />").val(country).text(country);
      if (chart.highlight == country) { el.attr("selected", true); }
      $highlight.append(el);
    }
  }
...
var process_data = function(data, chart) {
  ...
  for (var country in agg) {
    ...
    for (var i = 0; i < dates.length; i++) {
      ...
      if (!chart.growthFactor && dayCounter == -1 && cases >= chart.y0) {
        dayCounter = 0;
      } else if (dayCounter == -1 && chart.growthFactor && rawCaseValue >= 100) {
        dayCounter = 0;
      }
...
var render = function(chart) {
  ...
  var renderLineChart = function(svg, i) {
    ...
    svg.selectAll("countries")
      ...
      .attr("cy", function (d) {
        if (d.cases < scale_y0) { return -999; }
